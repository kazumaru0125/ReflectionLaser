...
public class EditSettingsWindow : EditorWindow
{
    ...
    private void OnGUI()
        ...
        if (serializedObject == null)
            {
            EditorGUILayout.HelpBox("GenerationSettings が設定されていません。\n引数付きの OpenWindow(GenerationSettings genSetts) を使ってください。", MessageType.Warning);
        ...
        if (GUILayout.Button("Generate", GUILayout.Width(Screen.width / 2), GUILayout.Height(Screen.height / 6)))
            {
            var generator = GameObject.FindObjectOfType<Generator>();
            if (generator)
                {
                if (generator.GenerationSettings)
                    {
